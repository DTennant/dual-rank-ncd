...
class CIFAR100_range(CIFAR100):
    def __init__(self, root, split='train+test',
                 transform=None, target_transform=None,
                 download=False, target_list = range(5)):
        # the labeled data will used to concat
        super().__init__(root, split, transform, target_transform, download, target_list)
        self.num_target_class = len(target_list)
        self.target_class_idx = {item: idx for idx, item in enumerate(target_list)} 

        def target_transform(target):
            if target in self.target_class_idx:
                return self.target_class_idx[target]
            else:
                assert target >= len(self.target_class_idx)
                return target
        self.target_transform = target_transform
...
def CIFAR100Data_range(root, split='train', aug=None, target_list=range(80)):
    if aug==None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.507, 0.487, 0.441), (0.267, 0.256, 0.276)),
    ...
    dataset = CIFAR100_range(root=root, split=split, transform=transform, target_list=target_list)
    return dataset
...
def CIFAR100LoaderMix_range(root, batch_size, split='train',num_workers=2, aug=None, shuffle=True, labeled_list=range(80), unlabeled_list=range(90, 100)):
    dataset_labeled = CIFAR100Data_range(root, split, aug, labeled_list)
    dataset_unlabeled = CIFAR100Data_range(root, split, aug, unlabeled_list)
    # make sure to use label to concat, not unlabel
    dataset_labeled.targets = np.concatenate((dataset_labeled.targets,dataset_unlabeled.targets))
    dataset_labeled.data = np.concatenate((dataset_labeled.data,dataset_unlabeled.data),0)
    loader = data.DataLoader(dataset_labeled, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
    return loader
