    ...
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='cifar10':
            ...
            self.data = datasets.__dict__[self.dataset_name.upper()](
                ...
                download=True, transform=self.transform)
        elif self.dataset_name == 'nori':
            from .nori_util import NoriDataset
            # nori will be used by imagenet100
            self.mean_pix = [0.485, 0.456, 0.406]
            self.std_pix = [0.229, 0.224, 0.225]
            ...
            transform.append(lambda x: np.array(x))
            self.transform = transforms.Compose(transform)
            self.data = NoriDataset(self.json_path, transform=self.transform, return_idx=False)
